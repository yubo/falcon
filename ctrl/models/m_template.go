...
func (u *User) GetTemplate(id int64) (*TemplateAction, error) {
	...
	if a, err := u.GetAction(ta.T.ActionId); err != nil {
		return nil, err
	} else {
		ta.A = *a
	}

	if t, err := u.getTemplate(ta.T.ParentId); err == nil {
		ta.Pname = t.Name
	}
...
func (u *User) UpdateTemplate(id int64, _o *TemplateAction) (o *Template, err error) {
	var t *Template
	t, err = u.updateTemplate(id, &_o.T)
	if err != nil {
		return nil, err
	}
	_, err = u.UpdateAction(t.ActionId, &_o.A)
	if err != nil {
		return nil, err
	}
	return t, err
