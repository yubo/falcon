...
const (
	eof           = 0
	MAX_CTX_LEVEL = 16
	MODULE_NAME   = "\x1B[37m[SERVICE_EXPR_PARSE]\x1B[0m "
)
...
begin:
	...
	for {
		...
		for text[0] == ' ' || text[0] == '\t' || text[0] == '\n' {
			p.ctx.pos += 1
			if p.ctx.pos == len(p.ctx.text) {
				glog.V(5).Infof("%s ctx level %d", MODULE_NAME, p.ctxL)
				if p.ctxL > 0 {
					p.ctxL--
					p.ctx = &p.ctxData[p.ctxL]
					goto begin
		...
		if f != nil {
			if val, ok := keywords[string(f)]; ok {
				p.ctx.pos += len(f)
				glog.V(5).Infof("%s find %s return %d\n", MODULE_NAME, string(f), val)
				return val
			}
		}
		...
		if f != nil {
			...
			glog.V(5).Infof("%s return TEXT(%s)", MODULE_NAME, string(p.t))
			return TEXT
		}
